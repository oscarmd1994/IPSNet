...
namespace Payroll.Controllers
{
    public class ControlPayrollController : Controller
    {
        ...
        public JsonResult MenuInit()
        {
            ...
            string nav = "<nav class='nav nav-pills flex-column flex-sm-row pb-4'>";
            ...
            foreach (var item in MmenuBean)
            {
                if (item.iParent == 0)
                {
                    nav += "<a class='flex-sm-fill text-sm-center btn btn-menu rounded-0' data-toggle='pill' onclick='change()' href='#pills-" + item.sNombre + "'> " + item.sNombre + " </a>";
                    ...
                    foreach (var subitem in submenus)
                    {
                        ...
                        {
                            subnavs += "<li class='nav-item'><a class='nav-link ' onclick='seeview(" + '"' + subitem.sUrl + '"' + ")' href='#' ><span class='" + subitem.sIcono + "'></span> " + subitem.sNombre + " </a></li>";
                        }
                    }
                    subnavs += "</ul>";
                    i++;
